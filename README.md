...
:warning: _图片需要提前下载到本地, 图片名称**不要**包含空格_, 强烈建议使用英文或数字命名，**不要**使用中文命名。 例如:  
...
---
```  
大家注意到图片设置在 `image: "assets/images/001.jpg"` 这里, 替换这张图片需要两步:  
1. 将图片上传到 `assets/images` 文件夹下
2. 替换 `001.jpg` 成为为你上传的图片名称  

如果你想替换头像图片, 需要两步:  
1. 将图片上传到 `assets/images/authors` 文件夹下  
2. 替换 `logo.jpg` 成为你上传的头像图片名称
